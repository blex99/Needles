...
public class PlayerController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        controls.Gameplay.Spikey.performed += ctx => ToggleSpikey(true);
    }
    ...
    void Update()
    {
        ...
        // move, if you're alive and not jumping
        ...
        // when stucking to walls, fall slowly and give chance to wall/ curtain jump
        if (isSpikey && rigidbodyStuckTo != null)
        {
            if (isNearStickyWall)
            {
                ...
                // wall jump
                if ((Input.GetKeyDown(GameManager.instance.jump) || controls.Gameplay.Jump.triggered) && // either
                    ((onWall("right") && move.x < 0) || (onWall("left") && move.x > 0)))
                {
            ...
            {
                // jump from curtain
                if (Input.GetKeyDown(GameManager.instance.jump) || controls.Gameplay.Jump.triggered)
                {
                    FindObjectOfType<AudioManager>().PlaySFX("Jump");
                    isNearCurtain = false;
    ...
    void HandleHealth()
    {
        ...
        if (velocity.y <= -9.8f && IsGrounded() && !onSoftSurface())
        {
            ...
            if (health > 0)
            {
                ...
                FindObjectOfType<AudioManager>().PlaySFX("Owch");
            }
    ...
    void ToggleSpikey(bool fromController)
    {
        if (fromController && !GameManager.instance.usingController) return;
        isSpikey = !isSpikey;
        animator.SetBool("isSpikey", isSpikey);
    }

    void Jump(bool fromController)
    {
